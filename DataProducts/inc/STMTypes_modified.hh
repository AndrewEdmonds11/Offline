...
namespace mu2e {
  ...
  struct STMTrigType {
  public:
    STMTrigType(uint16_t mode, uint16_t channel, uint16_t type, uint16_t flag) {
      _data = (mode) | (channel << 10) | type | flag;
    }
    ...
    STMDigiFlag flag() const { return STMDigiFlag((_data & 0)); }
