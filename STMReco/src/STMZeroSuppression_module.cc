  ...
  void STMZeroSuppression::beginJob() {
    ...
    for (const auto& digi : *digisHandle) {
      ...
      for (size_t i = 0; i < starts.size(); ++i) {
        ...
        STMDigi stm_digi(STMTrigType(digi.trigType().mode(), digi.trigType().channel().id(), STMDataType::kZeroSuppressed), digi.trigTime()+digi.trigTimeOffset()+start, 0, 0, STMDigiFlag::kOK, zp_adcs);
